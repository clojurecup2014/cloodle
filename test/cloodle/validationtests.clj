...
(facts "Option, movie selection option eg. must have name"
        (v/validate-options {:options [{:name "Terminator 2"} {:name "The Running Man"}]}) => '({} {})
        (v/validate-options {:options [{:name "Terminator 2"} {}]}) => '({} {[nil :name] #{"option must have name"}})
        (v/validate-options {:options [{:name "    "}]}) => '({["    " :name] #{"option must have name"}}))
